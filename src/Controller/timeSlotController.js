        ...
        return res
          ...
          .json(
            responseFormatter(
              {},
              400,
              "Each slot must have startTime and endTime"
            )
          );
      }
    ...
    if (timeSlotDoc) {
      for (const newSlot of slots) {
        const duplicate = timeSlotDoc.slots.find(
          (s) =>
            s.startTime === newSlot.startTime && s.endTime === newSlot.endTime
        );
        if (duplicate) {
          return res
            .status(409)
            .json(
              responseFormatter(
                {},
                409,
                `Slot ${newSlot.startTime} - ${newSlot.endTime} already exists`
              )
            );
        }
      }
    ...
    return res
      ...
      .json(
        responseFormatter(
          ...
          201,
          "Time slots created successfully under one ID"
        )
      );
  } catch (err) {
...
exports.getAvailableSlots = async (req, res) => {
  try {
    ...
    const isToday =
      requestedDate.toDateString() === getISTDateObject().toDateString();
    if (isToday) {
